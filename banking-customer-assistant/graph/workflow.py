...
def node_card(state: GraphState) -> dict:
    ...
    return {"agent": "Card Agent", "response": result["response"], "current_step": result["current_step"], "agent_result": result}
...
def node_dispute_investigate(state: GraphState) -> dict:
    ...
    if amount is not None:
        ...
        if not matches:
            return {"agent": "Dispute Agent", "response": "I couldn't find a transaction matching that amount on your account.", "current_step": "No Match"}
    else:
...
def _handle_pending(message: str, customer_id: str, pending: dict, context: dict | None = None) -> dict:
    ...
    if ptype == "card_last4":
        ...
        if not last4:
            ...
            return {"response": "I didn't catch the card number. Please share the last 4 digits.", "current_step": "Awaiting Card Number", "agent": "Card Agent", "route": "Card Agent", "risk": Risk.SENSITIVE_READ.value, "intent": intent}
        result = card_agent.handle(customer_id, intent, last4)
        return {"response": result["response"], "current_step": result["current_step"], "agent": "Card Agent", "route": "Card Agent", "risk": Risk.REVERSIBLE_ACTION.value, "agent_result": result, "intent": intent}
    ...
    if ptype == "dispute_pick":
        ...
        if not txn:
            if classify_intent(message) != "fallback":
                return _run_classified(message, customer_id, context)
            return {"response": "I couldn't match that to one of the transactions listed. Please reply with the exact transaction ID.", "current_step": "Multiple Matches", "agent": "Dispute Agent", "route": "Dispute Agent", "risk": Risk.SENSITIVE_READ.value, "agent_result": {"candidates": candidates}}
        existing = dispute_agent.by_transaction(customer_id, txn["id"])
        if existing:
            return {"response": f"This transaction already has an open dispute: {existing['id']} ({existing['status']}).", "current_step": "Already Disputed", "agent": "Dispute Agent", "route": "Dispute Agent", "risk": Risk.SENSITIVE_READ.value}
        return {"response": f"Transaction ID: {txn['id']}\n\nWhat seems to be wrong with this transaction?", "current_step": "Awaiting Issue Type", "agent": "Dispute Agent", "route": "Dispute Agent", "risk": Risk.SENSITIVE_READ.value, "agent_result": {"transaction": txn}}
